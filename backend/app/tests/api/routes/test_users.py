...
def test_create_user_new_email(
    ...
    with (
        ...
        created_user = r.json()
        user = crud_user.get_user_by_email(session=session, email=username)
        assert user
...
def test_get_existing_user(
    ...
    assert 200 <= r.status_code < 300
    api_user = r.json()
    existing_user = crud_user.get_user_by_email(session=session, email=username)
...
def test_retrieve_users(
    ...
    crud_user.create_user(session=session, user_create=user_in2)
...
def test_update_user_email_exists(
    ...
    password2 = random_lower_string()
    user_in2 = UserCreate(email=username2, password=password2)
    user2 = crud_user.create_user(session=session, user_create=user_in2)
...
def test_delete_user_current_super_user_error(
    ...
    super_user = crud_user.get_user_by_email(session=session, email=settings.FIRST_SUPERUSER)
    assert super_user
