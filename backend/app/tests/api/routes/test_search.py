...
def test_search_as_admin(
    ...
    user = user_crud.create(
        db,
        UserCreate(
            username="testuser", email=random_email(), password=random_lower_string()
        ),
    )
    ...
    poem_crud.create(db, PoemCreate(title="Test Poem", content="aaaa"))
    poem_crud.create(
        db, PoemCreate(title="Private Test Poem", content="bbbb", is_public=False)
    )
    create_random_poem(db)

    collection_crud.create(
        db, CollectionCreate(name="Test Collection", user_id=user.id)
    )
    collection_crud.create(
        db,
        CollectionCreate(
            name="Private Test Collection", user_id=user2.id, is_public=False
        ),
    )
    create_random_collection(db, user.id)
