...
class PoemParser: 
    ...
    def _parse_word_format(self, poem: str) -> str: 
        poem = re.sub(r'(?<!\\)\*\*(.*?)\*\*', r'<strong>\1</strong>', poem, flags=re.DOTALL)
    ...
    def _parse_text_align(self, poem: str) -> str:
        ...
        sub = r'<div style="direction:rtl;">\1</div>'
    ...
    def _adjust_indentation_right_align(self, poem:str) -> str:
        matches = re.finditer(r'<div style="direction:rtl;">.*?</div>($|<div style="text-align:)', poem, flags=re.DOTALL)
        indent_matches = re.findall(r'<div style="text-indent:\dem;">.*?</div>', poem, flags=re.DOTALL)
        
        for match in matches: 
            for i_match in indent_matches:
                if poem.index(i_match) > match.start() and poem.index(i_match) < match.end(): 
                    new_match = re.sub(r'<div style="text-indent:(\d)em;">(.*?)([^\w\s])</div>', r'<div style="text-indent:\1em;">\3\2</div>', i_match, flags=re.MULTILINE)
